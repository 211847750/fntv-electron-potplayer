    ...
    return new Promise(resolve => {
        const request = http.get(healthUrl, {
            ...
            response.on('end', () => {
                ...
                try {
                    resolve(isProxyHealthPayload(JSON.parse(Buffer.concat(chunks).toString('utf8'))));
                } catch {
                    resolve(false);
                }
            });
...
export async function waitForProxyHealth(
    secret: string,
    ...
    while (Date.now() < deadline) {
        if (await probeProxyHealth(secret, Math.min(intervalMs, 500))) return true;
